...
class UploadTab extends React.Component<{ currentUser }, inputProps>{
    ...
    handleFileChange = file => {
      if (file && (!this.state.meetingName || (this.state.audioFile && this.state.meetingName === this.state.audioFile.name))) {
        this.setState({ meetingName: file.name });
      } else if (!file && this.state.audioFile && this.state.meetingName === this.state.audioFile.name) {
    ...
    uploadToCloud() {
      ...
      return this.getSignedURL(metadata).then((signedURL: any) => {
      ...
      return(
        <form id="uploadAudioForm" className="p-5 shadow-lg card-m">
          <div className="form-group mb-5">
            <FileUpload onSelectFile={this.handleFileChange} file={this.state.audioFile}></FileUpload>
          </div>
          <div className="d-flex justify-content-between mb-5 mt-2">
            <div className="d-flex w-50 mr-5">
              ...
              <input className="form-control" type="text" value={this.state.meetingName} onChange={this.handleChangeMeetingName}/>
            </div>
            <div className="d-flex w-50">
              <label className="font-weight-bold w-50 mt-1">Meeting Organizer:</label>
              <input className="form-control" type="text" value={this.state.organizer} onChange={this.handleChangeOrganizer}/>
            </div>
          </div>
          ...
          <div className="form-group mb-5">
            ...
            <AsyncSelect defaultOptions isMulti
          ...
          <Button className="form-group" variant="contained" color="default" startIcon={<CloudUploadIcon />} onClick={this.handleSubmitForm}>
            Submit
          </Button>
        </form>
      );
